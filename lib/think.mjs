...
// max_tokens budgets reasoning AND the answer together, and reasoning
// expands to fill whatever it is given: measured runs consumed 697/700 and
// 1197/1200, truncating mid-thought and returning an empty string. These
// are deliberately far larger than any answer needs, so the model finishes
// reasoning and still has room to speak. Brevity comes from the prompt.
const BUDGET = { minimal: 2500, low: 5000, medium: 9000, high: 16000, xhigh: 24000 };
...
export async function think({ card, effort, signal }, env = process.env) {
  ...
  // An empty answer is a failure, not a success with nothing in it — the
  // glasses should fall back to the attached detail rather than blank out.
  if (!text) {
    return { ok: false, reason: 'empty',
             message: 'Model returned no text; the token budget was likely spent on reasoning.',
