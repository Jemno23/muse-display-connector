...
function memoryStore() {
  ...
  const rendered = new Map(); // ticket  -> when the client said it drew it
  ...
  return {
    ...
    async markRendered(ticket) { rendered.set(ticket, Date.now()); },
    async wasRendered(ticket) { return Boolean(rendered.get(ticket)); },
...
function redisStore(url, token) {
  ...
  return {
    ...
    async markRendered(ticket) {
      await cmd('SET', renderKey(ticket), String(Date.now()), 'EX', ANSWER_TTL);
    },
    async wasRendered(ticket) {
      return Boolean(await cmd('GET', renderKey(ticket)));
    },
    async getAnswer(ticket) {
