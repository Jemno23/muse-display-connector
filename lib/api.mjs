...
const WAIT_BOUND_MS = 18_000; // well inside maxDuration, so we always return JSON
...
async function callTool(store, session, name, args, log) {
  ...
  switch (name) {
    ...
    case 'wait_for_answer': {
      ...
      // "showing" now means the glasses confirmed they drew it, not merely
      // that it left the queue. Those are different facts and only the
      // client knows the second one; inferring it from delivery was a lie
      // that hid a card being consumed and never rendered.
      const drawn = await store.wasRendered(args.ticket);
      log(`PENDING ticket=${args.ticket} display=${drawn ? 'showing' : 'delivered'}`);
      return content({
        status: 'pending',
        ticket: args.ticket,
        display: drawn ? 'showing' : 'delivered',
        hint: drawn
          ? 'The card is on the display, unanswered. Call again to keep waiting.'
          : 'Collected by the display but not yet drawn. Call again shortly.'
      });
    }
    default:
...
// Cards are filtered at delivery, not at push, because staleness is only
// meaningful relative to when someone actually looks.
//
// Two failures this prevents, both observed on hardware: an approval from
// twenty minutes ago still sitting at the front of the queue and getting
// answered instead of the current one; and two cards with identical text
// stacked up, indistinguishable on the display, so a pinch resolves the
// wrong ticket. Newest wins, and the moment expires.
function freshen(cards, log) {
  const now = Date.now();
  const live = [];
  for (const c of cards) {
    if (now - (c.at || 0) > CARD_TTL_MS) {
      log(`EXPIRE  ticket=${c.ticket} age=${Math.round((now - (c.at || 0)) / 1000)}s`);
      continue;
    }
    live.push(c);
  }
  ...
  const seen = new Set();
  const out = [];
  for (let i = live.length - 1; i >= 0; i--) {   // walk newest first
    const c = live[i];
    const key = c.kind + '|' + (c.body || c.line || '');
    if (seen.has(key)) {
      log(`DEDUPE  ticket=${c.ticket} superseded by a newer identical card`);
      continue;
    }
    seen.add(key);
    out.unshift(c);
...
export async function handlePending(store, session, log = () => {}) {
  ...
  return json(200, { cards: freshen(cards, log) });
}
